const TAKE_BARCODES_STORAGE_KEY = 'takeScannedBarcodes';
...
const UI_TEXT = {
    ...
    checkoutFailedTitle: 'Ошибка выдачи / Checkout failed',
    checkoutSuccessTitle: 'Выдача выполнена / Checkout successful',
    checkoutSuccessText: 'Устройства успешно выданы. / Devices were checked out successfully.',
    checkoutSubmitFailedText: 'Не удалось отправить список устройств. / Unable to submit the device list.'
};

function saveScannedBarcodes() {
    localStorage.setItem(TAKE_BARCODES_STORAGE_KEY, JSON.stringify(scannedBarcodes));
}
...
function loadScannedBarcodes() {
    try {
        const savedBarcodes = JSON.parse(localStorage.getItem(TAKE_BARCODES_STORAGE_KEY) || '[]');
...
async function submitScannedBarcodes(event) {
    event.preventDefault();
    ...
    try {
        const response = await fetch('/submit_scan', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        if (!response.ok || !data.success) {
            showToast('error', UI_TEXT.checkoutFailedTitle, data.message);
            return;
        }

        showToast('success', UI_TEXT.checkoutSuccessTitle, data.message || UI_TEXT.checkoutSuccessText);
        scannedBarcodes = [];
        localStorage.removeItem(TAKE_BARCODES_STORAGE_KEY);
        renderScannedBarcodes();
        fetch('/send_arduino_signal_on', { method: 'POST' });
        ...
        console.error('Error:', error);
        showToast('error', UI_TEXT.checkoutFailedTitle, UI_TEXT.checkoutSubmitFailedText);
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    submitButton.addEventListener('click', submitScannedBarcodes);
